...
Four CLI commands, then one launch. No mid-flow restarts.

### Before you start

- [**OpenAI Codex CLI**](https://developers.openai.com/codex/quickstart) drives every command below. Install with `npm install -g @openai/codex` or `brew install codex`. Runs on macOS, Windows, and Linux.
- **OpenAI Codex desktop app** is where you actually use the plugin. Available on macOS and Windows from [developers.openai.com/codex](https://developers.openai.com/codex/quickstart). Linux users can run the Codex TUI (`codex` in a terminal) instead.
...
Type `/plugins`, select **Massive Market Data**, press Install, then type `/exit` to leave cleanly. `/exit` matters here — closing the terminal or Ctrl-C leaves Codex state mid-flush.
...
This writes a user-level entry to `~/.codex/config.toml` that overrides the plugin's placeholder with your key. Rotate later with `codex mcp remove massive` followed by the same `add` command with the new value.
...
The plugin and MCP server both load on first launch. The first MCP call takes 5-10 seconds while `uvx` downloads and caches `mcp_massive`; subsequent calls start in about a second.
...
Should show `massive` as `enabled`. In Codex, try:
